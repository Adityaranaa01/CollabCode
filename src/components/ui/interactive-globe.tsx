...
export function Component({
  ...
  // ✅ FIX: use a ref to hold the draw function so draw doesn't reference itself
  const drawCallbackRef = useRef<() => void>();
  ...
  const draw = useCallback(() => {
    ...
    // ✅ FIX: call through ref, not directly — breaks the circular self-reference
    animRef.current = requestAnimationFrame(() => drawCallbackRef.current?.());
  }, [dotColor, arcColor, markerColor, autoRotateSpeed, connections, markers]);

  // ✅ Keep the ref in sync with the latest draw function
  useEffect(() => {
