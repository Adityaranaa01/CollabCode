...
const navItems: SidebarItem[] = [
  ...
  {
    icon: <CreditCard className="w-[22px] h-[22px]" />,
    label: "Pricing",
    href: "/plans",
  },
  {
          ...
          <nav className="space-y-1">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <span
                    className={`transition-transform duration-200 group-hover:scale-110 ${isActive ? "text-primary" : "text-slate-500 group-hover:text-slate-300"}`}
                  >
                    {item.icon}
                  </span>
